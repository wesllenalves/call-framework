 ...
 * @license http://pro.workcontrol.com.br?p=9696 Daniel Soares
 */
class Read {
     ...
     */
    public function ExeRead($Termos = null, $ParseString = null) {
     ...
     * @return ARRAY $this = Array ResultSet
     */
    public function LinkResult($Coluna, $Valor, $Campos = null) {
        if ($Campos):
            $this->FullRead("SELECT {$Campos} FROM  {$this->Tabela} WHERE {$Coluna} = :value", "value={$Valor}");
        else:
            $this->ExeRead($this->Tabela, "WHERE {$Coluna} = :value", "value={$Valor}");
        endif;
